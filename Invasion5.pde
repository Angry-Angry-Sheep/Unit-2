...
void draw() {
  ...
  shootLaser(640,80,665,54, 2, false);
    shootLaser(640,80,665,84, 2, false);
    shootLaser(640,80,654,70, 2, false);
    shootLaser(640,80,682,70, 2, false);
    shootLaser(640,80,0,600, 5, true);
...
void shootLaser(float originX, float originY, float targetX, float targetY, float tx, boolean pulse){
  float t;
  if (pulse)
    t = + tx + (float)Math.sin(timer/10)*(tx/6);
  else
    t = tx;
  stroke(13,255,0,30);
  strokeWeight(t*5);
